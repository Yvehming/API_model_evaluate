...
def get_map(MINOVERLAP, draw_plot, path = './map_out'):
    ...
    with open(RESULTS_FILES_PATH + "/results.txt", 'w') as results_file:
        ...
        for class_index, class_name in enumerate(gt_classes):
            ...
            print_tp = tp[score05_idx]
            # print("(fp[score05_idx] + tp[score05_idx])", (fp[score05_idx] + tp[score05_idx]))
            print_total = np.maximum((fp[score05_idx] + tp[score05_idx]), 1)
            ...
            if len(prec)>0:
                print(text + "\t||\tscore_threshold=0.5 : " + "F1=" + "{0:.2f}".format(F1[score05_idx])\
                    + " ; Recall=" + "{0:.2f}%".format(rec[score05_idx]*100) + " ; Precision=" + "{0:.2f}%".format(prec[score05_idx]*100))
            else:
                print(text + "\t||\tscore_threshold=0.5 : F1=0.00% ; Recall=0.00% ; Precision=0.00%")
            print("tp:", print_tp)
            print("total:", print_total)
            ap_dictionary[class_name] = ap
            tp_dictionary[class_name] = print_tp
            fp_dictionary[class_name] = print_total - print_tp
    ...
    with open(RESULTS_FILES_PATH + "/results.txt", 'a') as results_file:
        ...
        for class_name in sorted(dr_classes):
            ...
            text += "when confidence>0.5:\n" + "(tp:" + str(tp_dictionary[class_name]) + ""
            text += ", fp:" + str(fp_dictionary[class_name]) + ")\n"
            results_file.write(text)
